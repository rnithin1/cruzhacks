...
cv.readImage('./kevin1.jpg', function(err, img) {
 if(err) throw err;
  ...
  //var hist = histogram; //histogram(img); //Put histogram 
  ...
  for_loop(H, 1, histogram.length - classes + 1, 1, histogram.length, index); // call for-loop

  


  //var thresholds = otsu(hist, nclasses); //Put Otsu

  var thresholdsResults = document.getElementById("thresholds-results");

  thresholdsResults.value = thresholds.toString();
  drawResult(thresholds, nclasses); //Put drawResult
...
function drawResult(thresholds, classes)
{
    ...
    var img = dst.toDataURL("image/png");
    console.log(img.src);
